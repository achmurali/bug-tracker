...
export const addBug = asyncHandler(async (req: Request, res: Response) => {
  const projectId = req.params.projectId;
  const [name, description, status, priority] = checkRequestBody(req.body, [
    "name",
    "description",
    "status",
    "priority",
  ...
  try {
    const bugId = uuid();
    const result = await client.query(addBugQuery, [
      projectId,
      req.user,
      bugId,
    ]);
    if (result.rowCount != 1) throw new Exception("Something went wrong", 500);
    const resultDetails = await client.query(addBugDetailsQuery, [
      bugId,
      name,
      description,
      req.user,
...
const updateValues = (body:any,row:any,user:number) => {
    ...
    newValues.priority = body.priority??row.priority;
    newValues.status = body.status??row.statuse;
